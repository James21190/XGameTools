...
namespace X3TCTools.Bases.StoryBase_Objects.Scripting.KCode.TC
{
    public partial class TCKCodeDissassembler : KCodeDissassembler
    {
        ...
        protected override FunctionCallData[] _Decompile(ref List<int> visitedAddresses)
        {
            ...
            while (true)
            {
                ...
                switch ((TCKCodeInstructionAddress)functionData.Address)
                {
                    case TCKCodeInstructionAddress.PopMultiple: StackOffset += (short)functionParams[0]; break;

                    case TCKCodeInstructionAddress.Jump: InstructionOffset = (int)functionParams[0]; break;
                    //case TCKCodeInstructionAddress.JumpIfTrue6: subCalls = dissassembler.Decompile((int)functionParams[1],StackOffset,ScriptStack, ref visitedAddresses); break; // Faulty
                    case TCKCodeInstructionAddress.JumpIfFalse6: subCalls = dissassembler.Decompile((int)functionParams[1], StackOffset, ScriptStack, ref visitedAddresses); break;
                    case TCKCodeInstructionAddress.JumpIfTrue: subCalls = dissassembler.Decompile((int)functionParams[0], StackOffset, ScriptStack, ref visitedAddresses); break;
