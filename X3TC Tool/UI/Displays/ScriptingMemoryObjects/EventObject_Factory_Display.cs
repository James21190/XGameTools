...
{
    public partial class EventObject_Factory_Display : Form
    {
        EventObject m_EventObject;
        IScriptMemoryObject_Factory m_FactoryData { get {
                IScriptMemoryObject_Factory obj;
                switch (GameHook.GameVersion)
                {
                    case GameHook.GameVersions.X3TC: obj = new ScriptMemoryObject_TC_Factory(); break;
                    case GameHook.GameVersions.X3AP: obj = new ScriptMemoryObject_AP_Factory(); break;
        ...
        public EventObject_Factory_Display()
        {
            InitializeComponent();

            for (int i = 0; i < GameHook.GetTypeDataCount((int)SectorObject.Main_Type.Factory); i++)
            {
                cmbSubType.Items.Add(SectorObject.GetSubTypeAsString(SectorObject.Main_Type.Factory, i));
            } 
        ...
        public void Reload()
        {
            ...
            var cargo = m_FactoryData.Cargo;
            Array.Sort(cargo);
            txtCargo.Text = string.Join("\n", cargo);
            cmbSubType.SelectedIndex = m_FactoryData.SubType;
        }
